...
//    	}
	...
	@Test(priority=9)
	public void Trsdata() throws InterruptedException
	{
		clickElementByXpath("//a[@title='TRS Interaction']", "TrsData");
		WebElement canvas = driver.findElement(By.xpath("//canvas"));
		Actions actions = new Actions(driver);

		// Get the location of the canvas element
		...
		int centerX = canvasLocation.getX() + canvasWidth / 2;
		int centerY = canvasLocation.getY() + canvasHeight / 2;

		// Define the offset from the center for the drag and drop operation
		int offsetX = 100; // Example offset from the center in the x-direction
		int offsetY = 100; // Example offset from the center in the y-direction
		...
		Thread.sleep(5000);
		clickElementByXpath("//a[@title='Save']", "TRS save");
		Thread.sleep(3000);
		clickElementByXpath("//a[@title='Close Registration']", "TRS close");
		System.out.println("TRS data saved successfully");
